...
const SPTable = () => {
  ...
  useEffect(() => {
    getXList(currency); // Fetch data based on the selected currency
  ...
  const handleCurrencyChange = (currencyName) => {
    ...
    setLoaded(false); // Reset loaded state before fetching new data
  };
  ...
  return (
    <>
      <div className="box is-mobile">
        ...
        <p className="title has-text-primary-10">{t('sellingTable.title')}</p>
        ...
        {loaded && xlist ? (
          <div>
            <table class="table is-fullwidth has-background-white is-centered">
  <thead>
    <tr>
      <th className="has-text-primary-10">{t('sellingTable.bankName')}</th>
      <th className="has-text-primary-10">{t('sellingTable.sellingPrice')}</th>
    </tr>
  </thead>
  <tbody class="table is-border">
    {xlist.map((x) => (
      <tr key={x.id}>
        ...
        <td>
          <p class="has-text-info">{x.selling_price}</p>
        </td>
      </tr>
    ))}
  </tbody>
